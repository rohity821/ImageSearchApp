//
//  ThreadSafePlistTask.swift
//  Wynk Music
//
//  Created by Ankit Gupta on 09/01/17.
...
public class ThreadSafePlistTask {
    ...
    private struct Constants {
        static let commonOperationQueue = DispatchQueue(label: "com.wynk.ThreadSafePlistTask.common.reader", qos: DispatchQoS.userInitiated, attributes: DispatchQueue.Attributes.concurrent, autoreleaseFrequency: DispatchQueue.AutoreleaseFrequency.inherit, target: nil)
        static let writerQueue = DispatchQueue(label: "com.wynk.ThreadSafePlistTask.common.write")
        ...
        if let plistPath = plistPath {
            Constants.writerQueue.sync(flags: .barrier) { [unowned self] in
                ...
                do {
                    ...
                    NSLog("ThreadSafePlistTask: \(plistPath) fileSize Error: \(error)")
                }
                
                if fileSize > 0 {
                    NSLog("ThreadSafePlistTask \(plistPath) \(fileSize / 1024 / 1024) MB")
                    if fileSize > Constants.maxPlistFileThresholdSize {
                        NSLog("ThreadSafePlistTask: \(plistPath) deleted due to over size")
                ...
                if let dict = NSDictionary(contentsOfFile: plistPath) as? Dictionary<String, AnyObject> {
                    ...
                    NSLog("ThreadSafePlistTask \(plistPath) \(dict.keys.count) count")
                } else {
                ...
                NotificationCenter.default.addObserver(self, selector: #selector(ThreadSafePlistTask.notificationApplicationWillResignActive), name:NSNotification.Name.UIApplicationDidEnterBackground, object: nil)
    ...
    deinit {
        NSLog("ThreadSafePlistTask deallocated \(plistPath ?? "")")
